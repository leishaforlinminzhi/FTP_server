...
int main() {
    ...
    */
    ...
    if ((sk = socket(PF_INET, SOCK_DGRAM, 0)) < 0) {
        printf("c Problem creating socket\n");
        exit(1);
    }
    ...
    */
    ...
    while (1) {
        ...
        if (n_sent < 0) {
            printf("c Problem sending data");
            exit(1);
        }

        if (n_sent != buf_len) {
            printf("c Sendto sent %d bytes\n", n_sent);
        }
